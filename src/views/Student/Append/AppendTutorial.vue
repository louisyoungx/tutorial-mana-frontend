<template>
    <container-body>
        ...
        <container-content>
            ...
            <content-full>
                <content-container>
                    <h1 class="font-medium text-gray-900 text-3xl mb-6">新的帮扶活动</h1>
            ...
            <content-full>
                <content-container title="参加的帮扶活动">
                    <tutorial-activity-card
                            ...
                            :showButton="true"
                            @accept="accept(active.id)"
                            @cancel="cancel(active.id)"
                    />
                </content-container>
            </content-full>
...
<script>
    ...
    export default {
        ...
        mounted () {
            this.tutorialActivity = this.deepClone(this.$store.state.student.tutorialUnSelected)
        },
        methods: {
            accept (id) {
                console.log(id)
                ...
                api.appendTutorial(data).then(res => {
                    this.updateStudent()
                })
            },
            cancel (id) {
            ...
            updateStudent() {
                api.update(this.$store.state.id, 'student').then(info => {
                    ...
                    this.tutorialActivity = this.deepClone(this.$store.state.student.tutorialUnSelected)
                    console.log(this.$store.state.student.tutorialUnSelected)
                })
            },
        }
    }
